...
export class mjBizAppsIssuesIssueComment_ {
    ...
    @Field({nullable: true, description: `The Issue this comment belongs to.`}) 
    @MaxLength(36)
    IssueID?: string;
        
    @Field({nullable: true, description: `Comment body (Markdown or plain text).`}) 
    Body?: string;
...
export class mjBizAppsIssuesIssueNumberSequence_ {
    ...
    @Field(() => Int, {nullable: true, description: `The next sequence value to assign for this scope. Incremented atomically (UPDLOCK/HOLDLOCK) by spAssignNextIssueNumber.`}) 
...
export class mjBizAppsIssuesIssueStatus_ {
    ...
    @Field({nullable: true, description: `Display name of the status (unique). E.g. 'In Progress', 'Resolved'.`}) 
    @MaxLength(100)
    Name?: string;
    ...
    @Field(() => Int, {nullable: true, description: `Sort order of the status on boards and in dropdowns. Lower values appear first.`}) 
    Sequence?: number;
        
    @Field(() => Boolean, {nullable: true, description: `Whether new issues default to this status. Exactly one status should have this set.`}) 
    IsDefault?: boolean;
        
    @Field(() => Boolean, {nullable: true, description: `Whether this is a terminal (end) state such as Closed or Won't Fix. Terminal statuses stop SLA timers and remove the issue from active queues.`}) 
    IsTerminal?: boolean;
    ...
    @Field(() => Boolean, {nullable: true, description: `Whether this is the resolved-but-not-closed state (e.g. Resolved). Entering an IsResolved status stamps Issue.ResolvedAt. Distinct from IsTerminal: an issue can be resolved while still open for confirmation before it is closed.`}) 
    IsResolved?: boolean;
...
export class mjBizAppsIssuesIssueType_ {
    ...
    @Field({nullable: true, description: `Display name of the issue type (unique). E.g. 'Bug', 'Feature Request'.`}) 
    @MaxLength(100)
    Name?: string;
    ...
    @Field({nullable: true, description: `Priority assigned to new issues of this type when none is specified. One of Low, Medium, High, Critical.`}) 
    @MaxLength(20)
    ...
    @Field(() => Boolean, {nullable: true, description: `Whether this issue type is available for new issues. Inactive types stay on historical issues but are hidden from selection.`}) 
    IsActive?: boolean;
...
export class mjBizAppsIssuesIssue_ {
    ...
    @Field({nullable: true, description: `Short, one-line summary of the issue.`}) 
    @MaxLength(500)
    Title?: string;
    ...
    @Field({nullable: true, description: `The IssueType classifying this issue. Drives lifecycle action hooks and the default spawned task type.`}) 
    @MaxLength(36)
    IssueTypeID?: string;
        
    @Field({nullable: true, description: `Current workflow status of the issue.`}) 
    @MaxLength(36)
    StatusID?: string;
        
    @Field({nullable: true, description: `Impact of the issue (how bad it is): Low, Medium, High, Critical. Distinct from Priority.`}) 
    @MaxLength(20)
    Severity?: string;
        
    @Field({nullable: true, description: `Scheduling priority (how soon to address it): Low, Medium, High, Critical. Distinct from Severity.`}) 
    @MaxLength(20)
    Priority?: string;
    ...
    @Field(() => Float, {nullable: true, description: `Predicted probability (0.0000 - 1.0000) that this issue escalates to critical or high back-and-forth severity.`}) 
    PredictedCriticalEscalationProbability?: number;
        
    @Field({nullable: true, description: `Operational escalation risk tier: Low (<0.30), Medium (0.30-0.70), High (>0.70), or Critical (>=0.90).`}) 
    @MaxLength(20)
    PredictedEscalationRiskBand?: string;
        
    @Field({nullable: true, description: `Timestamp when this issue was last scored by the predictive escalation model.`}) 
    PredictedEscalationScoredAt?: Date;
...
export class CreatemjBizAppsIssuesIssueInput {
    ...
    PredictedCriticalEscalationProbability: number | null;

    @Field({ nullable: true })
    PredictedEscalationRiskBand: string | null;

    @Field({ nullable: true })
    PredictedEscalationScoredAt: Date | null;
