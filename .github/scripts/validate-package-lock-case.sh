...
# Workspace paths live as keys of the top-level `importers:` map, one indent level in.
# Extract that block without needing a YAML parser: everything between `importers:` and
# the next column-0 key, keeping only 2-space-indented keys under packages/ or apps/.
